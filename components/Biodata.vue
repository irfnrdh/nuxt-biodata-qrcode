...
<template>
  ...
  >
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
      <div class="mt-8 bg-white overflow-hidden shadow sm:rounded-lg p-10">
        <h2 class="text-2xl leading-7 font-semibold text-center">Biodata</h2>
        <p class="mt-3 text-gray-600">
          <Gambar :width="350" :image-src="image" />
        </p>
        <p class="mt-3 text-gray-600">
          Id :
          <code class="bg-gray-100 text-sm p-1 rounded border">
            {{ this.$route.query.id }}</code
          >
        </p>
        <p class="mt-3 text-gray-600">
          Nama :
          <code class="bg-gray-100 text-sm p-1 rounded border">
            {{ this.$route.query.nama }}</code
          >
        </p>
        <p class="mt-3 text-gray-600">
          Tanggal Lahir :
          <code class="bg-gray-100 text-sm p-1 rounded border">
            {{ this.$route.query.tanggal }}</code
          >
        </p>
        <p class="mt-3 text-gray-600">
          Jabatan :
          <code class="bg-gray-100 text-sm p-1 rounded border">
            {{ this.$route.query.jabatan }}</code
          >
        </p>
      </div>
      <div class="flex justify-center pt-4 space-x-2">
        <a href="https://github.com/irfnrdh" target="_blank"
          ><svg
          ...
          >
            ...
            /></svg
        ></a>
        <a href="https://twitter.com/irfnrdh" target="_blank"
          ><svg
            class="w-6 h-6 text-gray-600 hover:text-gray-800"
            xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink"
